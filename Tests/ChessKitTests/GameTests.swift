...
class GameTests: XCTestCase {
    ...
    func testMoveVariations() {
        ...
        let nf3Index = MoveIndex(number: 2, color: .white, variation: 0)
        game.make(moves: ["Nc3", "Nf6", "Bc4"], from: nf3Index.previous)
        
        let nc3Index = MoveIndex(number: 2, color: .white, variation: 1)
        
        XCTAssertEqual(game.moves[nf3Index]?.san, "Nf3")
        XCTAssertEqual(game.moves[nc3Index]?.san, "Nc3")
        
        // add 2... Nc6 ... variation to 2... Nf6
        let nf6Index = MoveIndex(number: 2, color: .black, variation: 1)
        game.make(moves: ["Nc6", "f4"], from: nf6Index.previous)
        
        let nc6Index = MoveIndex(number: 2, color: .black, variation: 2)
        ...
        let nc6Index2 = MoveIndex(number: 2, color: .black, variation: 0)
        game.make(moves: ["f5", "exf5"], from: nc6Index2.previous)
        
        let f5Index = MoveIndex(number: 2, color: .black, variation: 3)
